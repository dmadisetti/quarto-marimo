...
local function is_mime_sensitive()
    ...
    return string.match(output_file, "%.pdf$") or
               string.match(output_file, "%.tex$")
end
local function run_marimo()
    ...
    if is_mime_sensitive(doc) then mime_sensitive = "yes" end
    ...
    for _, filename in ipairs(PANDOC_STATE.input_files) do
        ...
        if input_file then
            ...
            result = pandoc.json.decode(pandoc.pipe(endpoint_script,
...
function CodeBlock(el)
    ...
    if el.attr and
        el.attr.classes:find_if(
            function(c) return string.match(c, ".*marimo.*") end) then
        missingMarimoCell = false
        if cell_index > expected_cells then
            message = string.format("attempted to extract more cells than " ..
...
function Pandoc(doc)
    ...
    if expected_cells ~= cell_index - 1 then
        error("marimo filter failed. Expected " .. expected_cells ..
                  " cells, but got " .. cell_index - 1)
    end

    -- Don't do anything if we don't have to
    if missingMarimoCell then return doc end
