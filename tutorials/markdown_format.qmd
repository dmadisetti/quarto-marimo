...
`{.python.marimo}`, or `{.python note="Whatever you want"}`, marimo will treat it as a
Python cell.
<!---->
...
in `{.python.marimo}` code blocks. Although `{.python.marimo}` might be more concise,
...
```{.python.marimo unparsable="true" disabled="true"}
#| echo: true
# This won't actually run with islands. Disabled.
print("This code cell has a syntax error"
```
